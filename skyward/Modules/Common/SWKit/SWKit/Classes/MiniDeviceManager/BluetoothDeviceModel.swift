...
public struct Command {
    ...
    public static let platformReqLocation = Command(name: "平台读取终端当前定位信息",
                                                    chengduCode: 0x0013,
                                                    k01Code: 0x13)
...
public struct K01AlarmInfo {
    public var alarmType: AlarmType
    public var battery: Int?              // 仅当 alarmType 为 .sos 或 .safe 时有值
    public var longitudeDirection: Direction
    public var latitudeDirection: Direction
...
public enum AlarmType: Int {
    case sos = 0        // 开启SOS（报警）
    case safe = 1       // 报平安
    case cancelSOS = 2  // 关闭SOS
    
    public var description: String {
        switch self {
        case .sos: return "开启SOS（报警）"
        case .safe: return "报平安"
        case .cancelSOS: return "关闭SOS"
        }
    }
    
    /// 是否包含电量字段
    public var hasBattery: Bool {
        return self != .cancelSOS
    }
}
...
public struct K0115CommondResponseFrame {
    public let header: UInt16           // 帧头 0xAA55
    public let serialNumber: UInt8      // 流水码
    public let dataLength: UInt16       // 数据长度
    public let commandCode: UInt8       // 命令编号
