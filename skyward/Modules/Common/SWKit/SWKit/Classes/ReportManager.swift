...
public enum ReportType: String {
    ...
    public var reportStateTip: String {
        switch self {
        case .safety:
            return "报平安已上报"
        case .openSOS:
            return "SOS报警已上报，状态已开启"
        case .closeSOS:
            return "SOS报警状态已关闭"
        }
    }
        ...
        if hasNetwork {
            ...
            SWAlertView.showAlert(title: nil, message: tips) {
                if let _ = BluetoothManager.shared.connectedPeripheral {
                    switch type {
                    case .safety:
                        BluetoothManager.shared.sendSafe()
                        NotificationCenter.default.post(name: .offlineReportState, object: type)
                    case .openSOS:
                        BluetoothManager.shared.openSOS()
                    case .closeSOS:
                        BluetoothManager.shared.closeSOS()
