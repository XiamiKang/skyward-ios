...
public protocol RoutableType {
    ...
    static var priority: RouterPriority { get }
}
...
public protocol RoutableActionType: RoutableType {
    ...
    ///     false：表示该路由内部，不处理
    @discardableResult
    static func handle(_ url: URLConvertible, _ callback:((_ result:Any?) -> Void)?) -> Bool
}
...
extension RoutableType {
    ...
    public static var priority: RouterPriority {
        return .normal
    }
    ...
    public func show(_ fromViewController: UIViewController, completion: @escaping () -> Void) {
        ...
        case .push(let animated):
            if let delegate = self as? RoutableTypeDelegate {
                delegate.shouldShowController(controller, fromViewController: fromViewController, segueKind: self.segueKind) { shouldShow in
                    
                    guard shouldShow else { return }
                    
                    delegate.willShowController(controller, fromViewController: fromViewController, segueKind: self.segueKind)
                    fromViewController.navigationController?.pushViewController(controller, animated: animated)
                    delegate.didShownController(controller, fromViewController: fromViewController, segueKind: self.segueKind)
                }
            } else {
                fromViewController.navigationController?.pushViewController(controller, animated: animated)
            }
