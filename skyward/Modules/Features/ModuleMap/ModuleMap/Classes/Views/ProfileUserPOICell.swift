...
class ProfileUserPOICell: UITableViewCell {
    ...
    func config(with poiData: UserPOILocalData, isManageState: Bool = false, isSelected: Bool = false) {
        // 设置图标
        ...
        POIImageView.snp.updateConstraints { make in
            make.left.equalTo(checkboxImageView.snp.right).offset(isManageState ? 12 : -16)
        }
    }
}
