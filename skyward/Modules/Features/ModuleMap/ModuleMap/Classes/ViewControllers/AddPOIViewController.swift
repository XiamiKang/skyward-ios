...
class AddPOIViewController: UIViewController {
    ...
    // MARK: - 修改
    var poiEditData: UserPOILocalData?              // POI数据
    ...
    init(poiData: UserPOILocalData) {
        self.poiEditData = poiData
        if let imageData1 = poiData.imageData1 {
            imgUrlList.append(imageData1)
        }
        if let imageData2 = poiData.imageData2 {
            imgUrlList.append(imageData2)
        }
        if let imageData3 = poiData.imageData3 {
    ...
    private func updatePOIData() {
        if let poiEditData = poiEditData {
            nameTextField.text = poiEditData.name ?? ""
            selectedType = POIType.categoryConversionType(category: poiEditData.category ?? 1)
            addressNameLabel.text = poiEditData.address ?? ""
            let longitudeStr = String(format: "%.6f", poiEditData.lon ?? 00)
            let latitudeStr = String(format: "%.6f", poiEditData.lat ?? 00)
            addressLogAndLatLabel.text = "经纬度：\(longitudeStr)°E, \(latitudeStr)°N"
            addressAltitudeLabel.text = "海拔：\(poiEditData.altitude ?? 00)米"
        }
    ...
    private func savePOI() {
        ...
        if UserPOILocalDBManager.shared.insertOrUpdate(poiData: userPOILocalData) {
            ...
            viewModel.uploadLocalPOIDataToNetwork(userPOILocalData)
