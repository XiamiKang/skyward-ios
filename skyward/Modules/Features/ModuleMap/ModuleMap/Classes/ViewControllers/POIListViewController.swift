...
class POIListViewController: BaseViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if isManageState {
            ...
            let data = dataSource[indexPath.row]
            let vc = POIDetailViewController(poiData: data)
    ...
    private func deleteSingleItem(at indexPath: IndexPath) {
        ...
        if UserPOILocalDBManager.shared.delete(byId: data.id ?? 0) {
            ...
            _ = viewModel.deleteUserPoi(data.poiId ?? "")
