//
//  AddPOIViewController 2.swift
//  Pods
//
//  Created by TXTS on 2026/4/2.
//
...
// MARK: - 添加兴趣点页面
class EditPOIViewController: UIViewController {
    ...
    // MARK: - 修改
    var poiData: UserPOILocalData              // POI数据
    ...
    private lazy var addressLogAndLatLabel: UILabel = {
        let label = UILabel()
        label.text = "经纬度：--"
        let longitudeStr = String(format: "%.6f", poiData.lon ?? 00)
        let latitudeStr = String(format: "%.6f", poiData.lat ?? 00)
    ...
    init(poiData: UserPOILocalData) {
        ...
        if let imageData1 = poiData.imageData1, !imageData1.isEmpty {
            imgUrlList.append(imageData1)
        }
        if let imageData2 = poiData.imageData2, !imageData2.isEmpty {
            imgUrlList.append(imageData2)
        }
        if let imageData3 = poiData.imageData3, !imageData3.isEmpty {
            imgUrlList.append(imageData3)
        }
    ...
    private func updatePOIData() {
        nameTextField.text = poiData.name ?? ""
        updateNamePlaceholder()
        selectedType = POIType.categoryConversionType(category: poiData.category ?? 1)
        typeButtons.forEach { $0.isSelected = ($0.type == selectedType) }
        updateAddButtonState()
        addressNameLabel.text = poiData.address ?? ""
        let longitudeStr = String(format: "%.6f", poiData.lon ?? 00)
        let latitudeStr = String(format: "%.6f", poiData.lat ?? 00)
        addressLogAndLatLabel.text = "经纬度：\(longitudeStr)°E, \(latitudeStr)°N"
        addressAltitudeLabel.text = "海拔：\(poiData.altitude ?? 00)米"
        descriptionTextView.text = poiData.description
        updatePlaceholder()
    }
    ...
    private func savePOI() {
        ...
        let poiLon = poiData.lon ?? 00.00
        let poiLat = poiData.lat ?? 00.00
        ...
        if UserPOILocalDBManager.shared.update(poiData: userPOILocalData, byId: poiData.id ?? 0) {
            UIWindow.topWindow?.sw_showSuccessToast("更新兴趣点成功")
            viewModel.uploadLocalPOIDataToNetwork(userPOILocalData, type: "update")
            self.dismiss(animated: true)
        }
...
extension EditPOIViewController: UITextFieldDelegate {
...
extension EditPOIViewController: UITextViewDelegate {
...
extension EditPOIViewController: PHPickerViewControllerDelegate {
