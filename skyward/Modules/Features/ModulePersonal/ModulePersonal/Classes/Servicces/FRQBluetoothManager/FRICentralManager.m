...
#import "FRICentralManager.h"
#import "FRICallback.h"

@implementation FRICentralManager

#define currChannel [friSpeaker callbackOnCurrChannel]
    ...
    if (self) {
        ...
        if ([backgroundModes containsObject:@"bluetooth-central"]) {
            NSDictionary *options =@{CBCentralManagerOptionShowPowerAlertKey:@NO,
                                     CBCentralManagerOptionRestoreIdentifierKey:@"com.frq.bluetoothRestore"
        ...
        restoreConnectedPeripherals = [[NSMutableArray alloc] init];
    }
    return  self;
...
- (void)scanPeripherals {
    [centralManager scanForPeripheralsWithServices:[currChannel friOptions].scanForPeripheralsWithServices options:[currChannel friOptions].scanForPeripheralsWithOptions];
}

//连接Peripherals
- (void)connectToPeripheral:(CBPeripheral *)peripheral{
    [centralManager connectPeripheral:peripheral options:[currChannel friOptions].connectPeripheralWithOptions];
}
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central {
    ...
    //发送通知
    [[NSNotificationCenter defaultCenter]postNotificationName:FRINotificationAtCentralManagerDidUpdateState object:@{@"central":central}];
    
    switch (central.state) {
        case CBCentralManagerStateUnknown:
            FRILog(@">>>CBCentralManagerStateUnknown");
            break;
        case CBCentralManagerStateResetting:
            FRILog(@">>>CBCentralManagerStateResetting");
            break;
        case CBCentralManagerStateUnsupported:
            FRILog(@">>>CBCentralManagerStateUnsupported");
            break;
        case CBCentralManagerStateUnauthorized:
            FRILog(@">>>CBCentralManagerStateUnauthorized");
            break;
        case CBCentralManagerStatePoweredOff:
            FRILog(@">>>CBCentralManagerStatePoweredOff");
            break;
        case CBCentralManagerStatePoweredOn:
            FRILog(@">>>CBCentralManagerStatePoweredOn");
            [[NSNotificationCenter defaultCenter]postNotificationName:FRINotificationAtCentralManagerEnable object:@{@"central":central}];
...
- (void)centralManager:(CBCentralManager *)central willRestoreState:(NSDictionary *)dict {
    ...
    [restoreConnectedPeripherals removeAllObjects];
    
    FRILog(@"===== central restore %@ !=======",dict);
    NSArray<CBPeripheral *> *restorePeripherals = [dict objectForKey:CBCentralManagerRestoredStatePeripheralsKey];
    [restoreConnectedPeripherals addObjectsFromArray:restorePeripherals];
...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI {
    ...
    //日志
    //FRILog(@"当扫描到设备:%@",peripheral.name);
    [self addDiscoverPeripheral:peripheral];
    
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:FRINotificationAtDidDiscoverPeripheral
    ...
    if ([currChannel filterOnDiscoverPeripherals]) {
        if ([currChannel filterOnDiscoverPeripherals](peripheral.name,advertisementData,RSSI)) {
            if ([currChannel blockOnDiscoverPeripherals]) {
                [[friSpeaker callbackOnCurrChannel] blockOnDiscoverPeripherals](central,peripheral,advertisementData,RSSI);
            }
        }
    }
...
- (void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral {
    ...
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:FRINotificationAtDidConnectPeripheral
                                                       object:@{@"central":central,@"peripheral":peripheral}];
    
    //设置委托
    [peripheral setDelegate:self];
    
    //FRILog(@">>>连接到名称为（%@）的设备-成功",peripheral.name);
    ...
    if (needDiscoverServices) {
        [peripheral discoverServices:[currChannel friOptions].discoverWithServices];
...
- (void)centralManager:(CBCentralManager *)central didFailToConnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error {
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:FRINotificationAtDidFailToConnectPeripheral
                                                       object:@{@"central":central,@"peripheral":peripheral,@"error":error?error:@""}];
 
    //    FRILog(@">>>连接到名称为（%@）的设备-失败,原因:%@",[peripheral name],[error localizedDescription]);
...
- (void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error {
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:FRINotificationAtDidDisconnectPeripheral
                                                       object:@{@"central":central,@"peripheral":peripheral,@"error":error?error:@""}];
    
    //    FRILog(@">>>外设连接断开连接 %@: %@\n", [peripheral name], [error localizedDescription]);
    if (error)
    {
        FRILog(@">>> didDisconnectPeripheral for %@ with error: %@", peripheral.name, [error localizedDescription]);
    }
    ...
    if ([self findConnectedPeripherals].count == 0) {
        ...
        //    FRILog(@">>> stopConnectAllPerihperals");
    }
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverServices:(NSError *)error {
    ...
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:FRINotificationAtDidDiscoverServices
                                                       object:@{@"peripheral":peripheral,@"error":error?error:@""}];
    
    //  FRILog(@">>>扫描到服务：%@",peripheral.services);
    if (error) {
        FRILog(@">>>didDiscoverServices for %@ with error: %@", peripheral.name, [error localizedDescription]);
    }
    //回叫block
    ...
    if (needDiscoverCharacteristics) {
        for (CBService *service in peripheral.services) {
            [peripheral discoverCharacteristics:[currChannel friOptions].discoverWithCharacteristics forService:service];
        }
    }
}
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error {
    ...
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:FRINotificationAtDidDiscoverCharacteristicsForService
                                                       object:@{@"peripheral":peripheral,@"service":service,@"error":error?error:@""}];
    
    
    if (error) {
        FRILog(@"error didDiscoverCharacteristicsForService for %@ with error: %@", service.UUID, [error localizedDescription]);
        //        return;
    }
    //回叫block
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    //发出通知
    [[NSNotificationCenter defaultCenter]postNotificationName:FRINotificationAtDidUpdateValueForCharacteristic
                                                       object:@{@"peripheral":peripheral,@"characteristic":characteristic,@"error":error?error:@""}];
    
    if (error) {
        FRILog(@"error didUpdateValueForCharacteristic %@ with error: %@", characteristic.UUID, [error localizedDescription]);
        //        return;
    }
    //查找字段订阅
    if ([friSpeaker notifyCallback:characteristic]) {
        [friSpeaker notifyCallback:characteristic](peripheral,characteristic,error);
        return;
    }
    //回叫block
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverDescriptorsForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if (error) {
        FRILog(@"error Discovered DescriptorsForCharacteristic for %@ with error: %@", characteristic.UUID, [error localizedDescription]);
        //        return;
    }
    //回叫block
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForDescriptor:(CBDescriptor *)descriptor error:(NSError *)error {
    ...
    if (error) {
        FRILog(@"error didUpdateValueForDescriptor  for %@ with error: %@", descriptor.UUID, [error localizedDescription]);
        //        return;
    }
    //回叫block
...
- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    [[NSNotificationCenter defaultCenter]postNotificationName:FRINotificationAtDidWriteValueForCharacteristic object:@{@"characteristic":characteristic,@"error":error?error:@""}];
    
    //    FRILog(@">>>didWriteValueForCharacteristic");
    //    FRILog(@">>>uuid:%@,new value:%@",characteristic.UUID,characteristic.value);
...
- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForDescriptor:(CBDescriptor *)descriptor error:(NSError *)error {
    //    FRILog(@">>>didWriteValueForCharacteristic");
    //    FRILog(@">>>uuid:%@,new value:%@",descriptor.UUID,descriptor.value);
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateNotificationStateForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    [[NSNotificationCenter defaultCenter]postNotificationName:FRINotificationAtDidUpdateNotificationStateForCharacteristic object:@{@"characteristic":characteristic,@"error":error?error:@""}];
    
    FRILog(@">>>didUpdateNotificationStateForCharacteristic");
    FRILog(@">>>uuid:%@,isNotifying:%@",characteristic.UUID,characteristic.isNotifying?@"isNotifying":@"Notifying");
...
- (void)peripheralDidUpdateRSSI:(CBPeripheral *)peripheral error:(nullable NSError *)error {
    [[NSNotificationCenter defaultCenter]postNotificationName:FRINotificationAtDidReadRSSI object:@{@"peripheral":peripheral,@"RSSI":peripheral.RSSI,@"error":error?error:@""}];
    
    FRILog(@">>>peripheralDidUpdateRSSI -> RSSI:%@",peripheral.RSSI);
...
- (void)peripheral:(CBPeripheral *)peripheral didReadRSSI:(NSNumber *)RSSI error:(NSError *)error {
    [[NSNotificationCenter defaultCenter]postNotificationName:FRINotificationAtDidReadRSSI object:@{@"peripheral":peripheral,@"RSSI":RSSI?RSSI:@100,@"error":error?error:@""}];
    
    FRILog(@">>>peripheralDidUpdateRSSI -> RSSI:%@",RSSI);
    if ([currChannel blockOnDidReadRSSI]) {
...
- (NSArray<CBPeripheral *> *)allRestorePeripherals{
    return restoreConnectedPeripherals;
}
