//
//  SavePointTimeSelectionView.swift
//  ModulePersonal
//
...
class SavePointTimeSelectionView: UIView {
    
    weak var delegate: SavePointTimeSelectionViewDelegate?
    private var selectedReport: String = "30分钟"
    
    private let options = SavePointTimeHelper.allOptions
    ...
    @objc private func optionButtonTapped(_ sender: UIButton) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { [weak self] in
            ...
            let reportValue = SavePointTimeHelper.reportValue(from: self.selectedReport)
            self.delegate?.didSelectSavePointTime(reportValue)
            self.hide()
        }
    }
...
protocol SavePointTimeSelectionViewDelegate: AnyObject {
