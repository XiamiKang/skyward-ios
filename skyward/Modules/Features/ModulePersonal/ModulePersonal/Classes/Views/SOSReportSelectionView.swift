...
class SOSReportSelectionView: UIView {
    
    weak var delegate: SOSReportSelectionViewDelegate?
    private var selectedReport: String = "3分钟"
    
    private let options = SOSReportHelper.allOptions
    ...
    func show(in view: UIView, currentReport: Int) {
        self.selectedReport = SOSReportHelper.reportString(from: currentReport)
        updateSelectionUI()
    ...
    @objc private func optionButtonTapped(_ sender: UIButton) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { [weak self] in
            ...
            let reportValue = SOSReportHelper.reportValue(from: self.selectedReport)
            self.delegate?.didSelectSOSReport(reportValue)
            self.hide()
        }
    }
...
protocol SOSReportSelectionViewDelegate: AnyObject {
